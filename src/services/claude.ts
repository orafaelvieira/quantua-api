...
Retorne APENAS um JSON válido (sem markdown, sem \`\`\`) com EXATAMENTE esta estrutura. Evite REPETIR conteúdo entre seções — cada uma tem um papel distinto (veja as regras):
{
  ...
  "situacao": { "classificacao": "saudável|estável|atenção|pressão operacional|pressão financeira|crise", "racional": "<onde nasce a força ou a pressão (na operação ou na estrutura financeira), CONTANDO a trajetória do período e com cada indicador citado já explicado em linguagem de dono>" },
