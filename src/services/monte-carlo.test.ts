...
describe("rodarMonteCarlo", () => {
  it("reproduz a MESMA simulação com o mesmo seed (auditável)", () => {
    const a = rodarMonteCarlo(inputMc([{ id: "v1", alvo: "no", refId: "volume", sensibMin: -0.1, sensibMax: 0.1 }]));
    const b = rodarMonteCarlo(inputMc([{ id: "v1", alvo: "no", refId: "volume", sensibMin: -0.1, sensibMax: 0.1 }]));
    expect(a.ok && b.ok).toBe(true);
    expect(a.equity).toEqual(b.equity);
    const c = rodarMonteCarlo(inputMc([{ id: "v1", alvo: "no", refId: "volume", sensibMin: -0.1, sensibMax: 0.1 }], 60, 99));
    expect(c.equity).not.toEqual(a.equity);
  });

  it("nó premissa: a distribuição envolve a base e varia de verdade", () => {
    const r = rodarMonteCarlo(inputMc([{ id: "v1", alvo: "no", refId: "volume", sensibMin: -0.2, sensibMax: 0.2 }], 120));
  ...
  it("linha % da receita: custo maior derruba o equity (sorteio quase determinístico)", () => {
    // faixa degenerada força o fator: +20% de custo em TODOS os cenários
    const pior = rodarMonteCarlo(inputMc([{ id: "v1", alvo: "linhaPct", refId: "cmv", sensibMin: 0.1999999, sensibMax: 0.2 }], 5));
    const melhor = rodarMonteCarlo(inputMc([{ id: "v1", alvo: "linhaPct", refId: "cmv", sensibMin: -0.2, sensibMax: -0.1999999 }], 5));
  ...
  it("linha fixa (valorMensal) e WACC também respondem na direção certa", () => {
    const aluguelAlto = rodarMonteCarlo(inputMc([{ id: "v1", alvo: "linhaValor", refId: "aluguel", sensibMin: 0.4999999, sensibMax: 0.5 }], 5));
    expect(aluguelAlto.ok).toBe(true);
    expect(Math.max(...aluguelAlto.equity)).toBeLessThan(aluguelAlto.base.equityFinal);

    const waccAlto = rodarMonteCarlo(inputMc([{ id: "v1", alvo: "wacc", sensibMin: 0.4999999, sensibMax: 0.5 }], 5));
    expect(waccAlto.ok).toBe(true);
    expect(Math.max(...waccAlto.equity)).toBeLessThan(waccAlto.base.equityFinal);
  });

  it("g sorteado acima do WACC é limitado (aviso, nenhum cenário perdido)", () => {
    const r = rodarMonteCarlo(inputMc([{ id: "v1", alvo: "g", sensibMin: 6.9999999, sensibMax: 7 }], 5));
    expect(r.ok).toBe(true);
    expect(r.n).toBe(5);
  ...
  it("variável inexistente vira aviso; todas inválidas = erro claro", () => {
    const r = rodarMonteCarlo(inputMc([
    ...
    const todasRuins = rodarMonteCarlo(inputMc([{ id: "v1", alvo: "no", refId: "nao-existe", sensibMin: -0.1, sensibMax: 0.1 }]));
    expect(todasRuins.ok).toBe(false);
  });

  it("LHS estratifica: com poucos cenários a média fica mais perto da teórica que o MC puro", () => {
    ...
    const comLhs = rodarMonteCarlo(mkInput(true));
    const semLhs = rodarMonteCarlo(mkInput(false));
    ...
    const desvioBase = (r: ReturnType<typeof rodarMonteCarlo>) => Math.abs(mediaDe(r.equity) - r.base.equityFinal) / Math.abs(r.base.equityFinal);
    expect(desvioBase(comLhs)).toBeLessThan(0.01); // LHS: média colada na base
  });

  it("correlação +0,9 entre duas variáveis aparece nos fatores sorteados (tornado usa os mesmos)", () => {
    ...
    const sem = rodarMonteCarlo({ ...inputMc(vars, 300, 11) });
    const com = rodarMonteCarlo({ ...inputMc(vars, 300, 11), correlacoes: [{ a: "vol", b: "cmv", rho: 0.9 }] });
  ...
  it("correlações inconsistentes são encolhidas com aviso (não quebram)", () => {
    const vars: McVariavelSpec[] = [
  ...
  it("persistência: anos deixam de ser independentes (fatores vizinhos parecidos)", () => {
  ...
  it("PERT e lognormal mudam a forma (lognormal puxa a cauda p/ a direita)", () => {
    ...
    const logn = mk("lognormal");
    const tri = mk("triangular");
  ...
  it("tornado: a variável de maior sensibilidade domina a contribuição à variância", () => {
    const r = rodarMonteCarlo(inputMc([
  ...
  it("perf smoke: um cenário do motor custa pouco (simulação de 1000 é viável)", () => {
    const t0 = performance.now();
    rodarMonteCarlo(inputMc([
