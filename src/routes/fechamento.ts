...
router.get("/historico-financeiro", async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  // A MONTAGEM mora em services/base-contabil.ts — esta rota só resolve acesso
  // e resposta. Cache e single-flight moram LÁ, junto da montagem: o IBR e o
  // seletor de documentos pedem a mesma base e precisam aproveitar o mesmo
  // trabalho (o fold é 20s+ de CPU numa instância única).
  const insumos = await insumosDaBase(companyId, req.scopeUserIds!);
  res.json(await montarBaseContabil(companyId, req.scopeUserIds!, insumos));
});
